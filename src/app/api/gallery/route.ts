...
export async function GET() {
  try {
    const result = await db.execute("SELECT * FROM gallery ORDER BY id ASC");
    ...
    console.error("Turso gallery fetch failed, falling back to static content:", error);
...
export async function POST(request: Request) {
  try {
    ...
    const result = await db.execute({
      ...
      args: [title, category, image, gridClass],
    });
